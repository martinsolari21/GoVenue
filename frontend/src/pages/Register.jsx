...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setCargando(true);
    try {
      const res = await authService.register({ nombre: form.nombre, email: form.email, password: form.password });
      ...
      <div className="auth-page">
        ...
        <div className="auth-center">
          <div className="auth-box">
            ...
            <p className="auth-sub">Registrate y empezá a publicar eventos</p>
            {error && <div className="auth-error">{error}</div>}
            <form onSubmit={handleSubmit}>
              <div className="auth-field">
                <label className="auth-label">Nombre</label>
                <input className="auth-input" type="text" placeholder="Tu nombre" value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} required />
              </div>
              <div className="auth-field">
              ...
              <div className="auth-field">
                ...
                <input className="auth-input" type="password" placeholder="Mínimo 6 caracteres" value={form.password} onChange={e => setForm({...form, password: e.target.value})} required />
              </div>
              <div className="auth-field">
                <label className="auth-label">Confirmar contraseña</label>
                <input className="auth-input" type="password" placeholder="Repetí la contraseña" value={form.confirmar} onChange={e => setForm({...form, confirmar: e.target.value})} required />
              </div>
              <button className="auth-btn" disabled={cargando}>{cargando ? 'Creando cuenta...' : 'Crear cuenta →'}</button>
            </form>
