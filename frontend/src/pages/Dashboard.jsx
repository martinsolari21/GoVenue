    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .db-header { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 2rem; }
        .db-title { font-family: 'Barlow Condensed', sans-serif; font-size: 2.8rem; font-weight: 900; text-transform: uppercase; letter-spacing: -1px; }
        ...
        .db-btn-crear-empty { background: #22c55e; color: #000; border: none; cursor: pointer; font-family: 'Barlow Condensed', sans-serif; font-size: 1rem; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; padding: 0.8rem 1.8rem; border-radius: 8px; margin-top: 1rem; transition: background 0.2s; }
        ...
        .db-evento-row { background: #0a0a0a; padding: 1.4rem 1.8rem; display: flex; align-items: center; gap: 1.5rem; transition: background 0.2s; }
        ...
        .db-btn-publicar { background: none; border: 1px solid rgba(34,197,94,0.3); cursor: pointer; color: #22c55e; font-family: 'Barlow Condensed', sans-serif; font-size: 0.85rem; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; padding: 0.4rem 0.9rem; border-radius: 6px; transition: all 0.2s; white-space: nowrap; }
        .db-btn-publicar:hover { background: rgba(34,197,94,0.1); }
      ...
      <div className="db-page">
        ...
        <div className="db-wrap">
          ...
          <div className="db-tabs">
            <button className={`db-tab ${tab === 'eventos' ? 'activo' : ''}`} onClick={() => setTab('eventos')}>Mis eventos</button>
            <button className={`db-tab ${tab === 'crear' ? 'activo' : ''}`} onClick={() => { setTab('crear'); setError(null); setExito(null); }}>+ Crear evento</button>
          </div>
                  ...
                  return (
                    <div key={ev.id} className="db-evento-row">
                      <div className="db-evento-info">
                        ...
                        <div className="db-evento-meta">
                          {ev.deporte?.nombre} · {formatFecha(ev.fecha)} · {ev.horaInicio}–{ev.horaFin} · {ev.venue?.nombre}
                        </div>
                      </div>
                      <span className="db-estado-badge" style={{background: est.bg, border: `1px solid ${est.border}`, color: est.color}}>
                        {ev.estado}
                      </span>
                      {ev.estado === 'BORRADOR' && (
                        <button className="db-btn-publicar" onClick={() => handlePublicar(ev.id)}>Publicar</button>
                      )}
                    </div>
                  );
                })}
          ...
          {/* TAB CREAR */}
          {tab === 'crear' && (
            <form className="db-form" onSubmit={handleCrearEvento}>
              ...
              <div className="db-form-row">
                <div className="db-field">
                  ...
                  <select className="db-select" value={form.deporteId} onChange={e => setForm({...form, deporteId: e.target.value})} required>
              ...
              <div className="db-form-row">
                ...
                <div className="db-field">
                  <label className="db-label">Precio (dejar vacío si es gratis)</label>
              ...
              <div className="db-form-actions">
                ...
                <button type="button" className="db-btn-cancel" onClick={() => { setForm(FORM_INICIAL); setTab('eventos'); }}>Cancelar</button>
              </div>
            </form>
          )}
        </div>
